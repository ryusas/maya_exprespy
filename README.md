...
Python のエクスプレッションコードは exec() 関数によって実行される仕組みなため、実行効率では劣ります。
...
##ディレクトリ構成
* examples: Mayaシーンの例。
* plug-ins: ビルド済みプラグインバイナリ。
* python: サポート python モジュール。
* scripts: サポート mel スクリプト。
* srcs: C++ソースコード。
...
* コードはどのように保存されるのか
  ...
  exprespy ノードの code という string アトリビュートに保存されます。
  コード内のアトリビュート参照は ```IN[index]``` や ```OUT[index]``` のようなプレースホルダに置き換わります。
  実際のアトリビュートは、それらに対応した input[] と output[] のプラグに接続されます。
